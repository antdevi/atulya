    ...
    async def prepare_state(self, reset=False):
        ...
        if not self.state or reset:
            ...
            if self.atulya.config.code_exec_ssh_enabled:
                pswd = self.atulya.config.code_exec_ssh_pass if self.atulya.config.code_exec_ssh_pass else await rfc_exchange.get_root_password()
        ...
        for i in range(2):
            try:
                ...
                PrintStyle(background_color="white", font_color="#1B4F72", bold=True).print(
                    f"{self.atulya.atulya_name} code execution output"
                )
