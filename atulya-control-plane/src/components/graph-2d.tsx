...
export function Graph2D({
  ...
  const [hoveredNodeStats, setHoveredNodeStats] = useState<{
    label: string;
    accessCount: number;
    connections: number;
  ...
  const cyElements = useMemo(() => {
    // Precompute per-node stats once (reuse in sizing + hover UI).
    const nodeConnections = new Map<string, number>();
    const nodeTotalLinkWeight = new Map<string, number>();
    graphData.links.forEach((link) => {
      ...
      nodeTotalLinkWeight.set(link.source, (nodeTotalLinkWeight.get(link.source) || 0) + weight);
      nodeTotalLinkWeight.set(link.target, (nodeTotalLinkWeight.get(link.target) || 0) + weight);
    });
    ...
    const nodes = graphData.nodes.map((node) => {
      ...
      return {
        data: {
          ...
          accessCount: Number((node.metadata as any)?.access_count ?? 0),
          totalLinkWeight: nodeTotalLinkWeight.get(node.id) || 0,
        },
      };
    });
  ...
  const dataSignature = useMemo(() => {
    return JSON.stringify({
      ...
      nodeIds: graphData.nodes
        ...
        .sort()
        .join(","),
      // Include style-relevant node fields so visual changes re-render even when
      // node/edge counts stay the same.
      nodeStyleSignature: graphData.nodes
        .map(
          (n) =>
            `${n.id}|${n.color ?? ""}|${n.size ?? ""}|${String((n.metadata as any)?.access_count ?? "")}`
        )
        .sort()
        .join(","),
      // Include style-relevant link fields for robust updates.
      linkStyleSignature: graphData.links
        .map(
          (l) =>
            `${l.source}|${l.target}|${l.type ?? ""}|${l.color ?? ""}|${l.width ?? ""}|${l.weight ?? ""}`
        )
        .sort()
        .join(","),
      showLabels,
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      if (cyRef.current && !cyRef.current.destroyed()) {
        ...
        // If counts are the same, update elements in-place instead of destroying/recreating.
        if (currentNodes === newNodes && currentEdges === newEdges) {
          console.log("Graph element counts unchanged, updating elements in-place");
          cyRef.current.batch(() => {
      ...
      try {
        ...
        cy.on("mouseover", "node", (evt: any) => {
          ...
          setNodeTooltipPos({ x: renderedPos.x, y: renderedPos.y });
          setHoveredNodeStats({
            label: node.data("label") || originalNode?.label || originalNode?.id || "Node",
            accessCount: Number(node.data("accessCount") || 0),
            connections: Number(node.data("connections") || 0),
            totalLinkWeight: Number(node.data("totalLinkWeight") || 0),
          });
  ...
  return (
    ...
    >
      ...
      {/* Link hover tooltip */}
      {hoveredNodeStats && nodeTooltipPos && (
        <div
        ...
        >
          ...
          >
            <div className="font-medium mb-1 truncate max-w-[220px]">{hoveredNodeStats.label}</div>
            <div className="opacity-85">Usage: {hoveredNodeStats.accessCount.toLocaleString()}</div>
            <div className="opacity-85">
              Connections: {hoveredNodeStats.connections.toLocaleString()}
            </div>
            <div className="opacity-85">
              Link weight: {hoveredNodeStats.totalLinkWeight.toFixed(3)}
            </div>
          </div>
        </div>
      )}
...
export function convertAtulyaGraphData(atulyaData: {
  ...
  const nodes: GraphNode[] = (atulyaData.nodes || []).map((n) => {
    ...
    return {
      ...
      metadata: {
        ...(tableRow || {}),
        access_count: n.data.accessCount ?? tableRow?.access_count ?? 0,
      },
    };
  });

  const links: GraphLink[] = (atulyaData.edges || []).map((e) => ({
